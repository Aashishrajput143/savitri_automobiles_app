...
class SalesHomeCubit extends Cubit<SalesHomeStates> {
  ...
  SalesHomeCubit()
      : recentlyAdded = [
        ...
        super(const SalesHomeInitial());

  Future<void> addSalesEntry() async {
    emit(const SalesHomeLoading());
    try {
      await Future.delayed(const Duration(seconds: 2));

      // Simulate navigation trigger
      emit(const SalesHomeSuccess(message: 'Navigate to next screen'));
    } catch (e) {
      emit(SalesHomeError(message: 'An error occurred: ${e.toString()}'));
    }
  }
